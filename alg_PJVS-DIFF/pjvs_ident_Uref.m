...
function Uref = pjvs_ident_Uref(s_mean, Uref1period, dbg);
    ...
    if dbg.v
        ...
        if dbg.pjvs_ident_Uref_phase
            ...
            legend('Calculated distances', 'selected phase', 'location', 'southoutside', 'orientation', 'horizontal')
        ...
        if dbg.pjvs_ident_Uref_all
            figure('visible',dbg.showplots)
            hold on
            % if ratio of s_mean vs Uref1period is more than 10x, multiply
            % s_mean to get plot easy to read.
            ratio = abs(max(s_mean)./max(Uref1period));
            if ratio > 10 || ratio < 0.1
                mul = 10^(round(log10(ratio)));
            end
            plot(s_mean./mul, '-x')
            plot(Uref1period, '+')
            plot(Uref, 'o')
            ylabel('Voltage (V)')
            xlabel('Sample index')
            legend(sprintf('Segment average * %g', 1/mul), ...
                   'PJVS reference values for 1 period', ...
                   'Replicated and matched reference values')
