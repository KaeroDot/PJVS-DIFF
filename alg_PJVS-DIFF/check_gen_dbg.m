...
function dbg = check_gen_dbg(dbg)
    ...
    dbg = check_set_vec(dbg, 'section', 0); % this is used only for plotting, to store actual section of the data
    ...
    dbg = check_set_bool(dbg, 'rms_values', debug_level - debug_offset); % plot calculated rms values of DUT (differentiall method)
    ...
    dbg = check_set_bool(dbg, 'sine_fit', debug_level - debug_offset); % plot sine fit of the waveform (differentiall method)
    dbg = check_set_bool(dbg, 'rms_plot', debug_level - debug_offset); % plot fft spectrum of the waveform (differentiall method)
...
function dbg = check_set_vec(dbg, fieldname, defaultvalue) %<<<1
% Checks if fieldname exist, if so, ensure it is vector.
    defaultvalue = int8(0);
    if ~isfield(dbg, fieldname)
        dbg.(fieldname) = defaultvalue;
    else
        if ~isvector(dbg.(fieldname))
            dbg.(fieldname) = dbg.(fieldname)(:, 1);
        end
    end
end % function dbg, check_set_int8(dbg, fieldname, defaultvalue)
